...
@NamedQueries({
	...
	@NamedQuery(
			...
			query = "select t from LocalizedString t where t.identifier = :" + LocalizedString.PARAM_IDENTIFIER + " and t.locale = :" + LocalizedString.PARAM_LOCALE + " and t.id = " +
			"(select max(tt.id) from LocalizedString tt where t.key = tt.key and tt.identifier = :" + LocalizedString.PARAM_IDENTIFIER + " and tt.locale = :" + LocalizedString.PARAM_LOCALE +
			" and tt.deleted = 0) and t.deleted = 0"
	),
	@NamedQuery(
	...
	@NamedQuery(
			...
			" and t.id = (select max(tt.id) from LocalizedString tt where t.key = tt.key and tt.identifier = :" + LocalizedString.PARAM_IDENTIFIER + " and tt.locale = :" + LocalizedString.PARAM_LOCALE +
			" and tt.deleted = 0) and t.deleted = 0"
	),
	@NamedQuery(
...
public class LocalizedString implements Serializable {
	...
	@Column(name = COLUMN_KEY, length = 4000, nullable = false)
	private String key;
